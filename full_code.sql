...
INSERT INTO RAW_SALES_ORDERS 
(order_id, customer_id, dc_id, order_date, order_source, order_status, ship_to_city, ship_to_state, ship_to_zip, sales_rep_id, total_amount)
...
CROSS JOIN (
    SELECT DATEADD(day, ROW_NUMBER() OVER (ORDER BY NULL) - 1, '2024-01-01')::DATE AS order_date
    FROM TABLE(GENERATOR(ROWCOUNT => 806))
) dates

CROSS JOIN TABLE(GENERATOR(ROWCOUNT => 15))

WHERE dates.order_date <= CURRENT_DATE()
...
INSERT INTO RAW_ERP_PURCHASE_ORDERS 
(po_id, po_number, vendor_id, dc_id, order_date, expected_delivery_date, po_status, payment_terms, buyer_id, approval_status, total_amount)

SELECT 
    'PO' || LPAD(ROW_NUMBER() OVER (ORDER BY order_date)::VARCHAR, 8, '0'),
    'PO-' || TO_CHAR(order_date, 'YYYYMMDD') || '-' || 
        LPAD(ROW_NUMBER() OVER (PARTITION BY order_date ORDER BY RANDOM())::VARCHAR, 3, '0'),
    v.vendor_id,
    dc.dc_id,
    order_date,
...
CROSS JOIN (
    SELECT DATEADD(day, SEQ4() * 7, '2024-01-01')::DATE AS order_date
    FROM TABLE(GENERATOR(ROWCOUNT => 115))
) dates

LEFT JOIN RAW_ERP_VENDOR_TERMS vt 
    ON v.vendor_id = vt.vendor_id 
    AND vt.sku_category_id = 'PASSENGER'

WHERE order_date <= CURRENT_DATE()
AND UNIFORM(1, 100, RANDOM()) <= 25;

-- 3.14 PO Line Items
INSERT INTO RAW_ERP_PO_LINE_ITEMS 
(po_line_id, po_id, sku_id, ordered_qty, unit_cost, extended_cost, line_status, requested_date, confirmed_date, promise_date)

SELECT 
    po.po_id || '-' || LPAD(ROW_NUMBER() OVER (PARTITION BY po.po_id ORDER BY RANDOM())::VARCHAR, 2, '0') AS po_line_id,
    po.po_id,
    p.sku_id,
    ...
    CASE 
        WHEN vt.volume_discount_tier2_qty IS NOT NULL THEN 
            CEIL(
                (
                    vt.min_order_qty +
                    (vt.volume_discount_tier2_qty - vt.min_order_qty) * UNIFORM(0::FLOAT,1::FLOAT,RANDOM())
                ) / vt.order_multiple
            ) * vt.order_multiple
    ...
    'OPEN' AS line_status,
    po.order_date AS requested_date,
    DATEADD(day, 2, po.order_date) AS confirmed_date,
    po.expected_delivery_date AS promise_date

FROM RAW_ERP_PURCHASE_ORDERS po
JOIN RAW_ERP_VENDORS v 
    ON po.vendor_id = v.vendor_id

CROSS JOIN (
    SELECT * 
    FROM RAW_MDM_PRODUCTS 
    ORDER BY RANDOM() 
    LIMIT 5
) p
...
LEFT JOIN RAW_PRICE_COST_HISTORY ch 
    ON p.sku_id = ch.sku_id
...
UPDATE RAW_ERP_PO_RECEIPTS r
SET 
    variance_qty = pol.ordered_qty - r.received_qty,
    variance_reason = CASE 
        WHEN pol.ordered_qty - r.received_qty > 0 THEN 'SHORT_SHIP'
        WHEN pol.ordered_qty - r.received_qty < 0 THEN 'OVER_SHIP'
        ELSE 'MATCH'
    END
FROM RAW_ERP_PO_LINE_ITEMS pol
WHERE r.po_line_id = pol.po_line_id;

-- 3.16 Supplier Scorecards
INSERT INTO RAW_SUPP_SCORECARDS 
(scorecard_id, vendor_id, evaluation_month, on_time_delivery_pct, quality_score, fill_rate_pct, overall_score)
...
CROSS JOIN (
    SELECT 
        DATE_TRUNC('month', DATEADD(month, -SEQ4(), CURRENT_DATE()))::DATE AS month_date
    FROM TABLE(GENERATOR(ROWCOUNT => 24))
) months;


-------------need to execute from here.
-- =====================================================================================
...
INSERT INTO EXT_WEATHER_HISTORICAL (
    weather_id, date, zip_code, city, state,
    temp_high_f, temp_low_f,
    precipitation_in, snowfall_in,
    wind_speed_mph, condition_code, weather_severity_index
)
...
SELECT 
    ...
    CASE 
        ...
        ELSE 1
    END AS weather_severity_index

FROM (
    -- 🔹 Base layer (ALL columns must be defined here)
    SELECT 
        'W-' || dc.zip_code || '-' || TO_CHAR(date_val, 'YYYYMMDD') AS weather_id,
        date_val AS date,
        dc.zip_code,
        dc.city,
        dc.state,
        ...
        CASE 
            WHEN dc.state IN ('AZ', 'TX', 'FL', 'CA') AND MONTH(date_val) IN (6, 7, 8)
                THEN UNIFORM(95, 115, RANDOM())
            WHEN dc.state IN ('CO', 'MN', 'MI') AND MONTH(date_val) IN (12, 1, 2)
                THEN UNIFORM(20, 40, RANDOM())
            WHEN MONTH(date_val) IN (6, 7, 8)
                THEN UNIFORM(75, 95, RANDOM())
            WHEN MONTH(date_val) IN (12, 1, 2)
                THEN UNIFORM(30, 55, RANDOM())
            ELSE UNIFORM(55, 75, RANDOM())
        END::DECIMAL(5,1) AS temp_high_f,

        0 AS temp_low_f,

        CASE 
            WHEN dc.state IN ('WA', 'OR') THEN UNIFORM(0, 0.5, RANDOM())
            WHEN dc.state IN ('FL', 'LA') AND MONTH(date_val) IN (6, 7, 8, 9)
                THEN UNIFORM(0, 1.5, RANDOM())
            ELSE UNIFORM(0, 0.3, RANDOM())
        END::DECIMAL(5,2) AS precipitation_in,

        -- ✅ IMPORTANT: must exist here
        CASE 
            WHEN dc.state IN ('CO', 'MN', 'MI', 'MA', 'NY') 
                 AND MONTH(date_val) IN (11, 12, 1, 2, 3)
                THEN UNIFORM(0, 8, RANDOM())
            ELSE 0
        END::DECIMAL(5,2) AS snowfall_in,

        UNIFORM(5, 25, RANDOM())::DECIMAL(5,1) AS wind_speed_mph

    FROM TIRECO_DW.BRONZE.RAW_WMS_DISTRIBUTION_CENTERS dc

    CROSS JOIN (
        SELECT DATEADD(day, SEQ4(), '2024-01-01') AS date_val
        FROM TABLE(GENERATOR(ROWCOUNT => 806))
    ) dates
);
...
INSERT INTO EXT_WEATHER_FORECAST (
    forecast_id, forecast_date, forecast_generated_at,
    zip_code, city, state,
    temp_high_f, temp_low_f,
    precip_probability_pct, snow_probability_pct,
    expected_snow_in, condition_code,
    severe_weather_alert, forecast_confidence
)
...
SELECT 
    ...
    CASE 
        WHEN expected_snow_in > 8 
             OR (precip_probability_pct > 80 AND state IN ('FL','LA','TX'))
        THEN TRUE ELSE FALSE 
    END AS severe_weather_alert,
...
FROM (
    -- 🔹 Base layer
    SELECT 
        'WF-' || dc.zip_code || '-' || TO_CHAR(DATEADD(day, day_offset, CURRENT_DATE()), 'YYYYMMDD') AS forecast_id,
        DATEADD(day, day_offset, CURRENT_DATE()) AS forecast_date,
        ...
        UNIFORM(10, 60, RANDOM()) AS precip_probability_pct,

        CASE 
            WHEN dc.state IN ('CO','MN','MI','MA','NY','WA') 
                THEN UNIFORM(20, 70, RANDOM())
            ELSE UNIFORM(0, 10, RANDOM())
        END AS snow_probability_pct,

        CASE 
            WHEN dc.state IN ('CO','MN','MI') 
                 AND UNIFORM(1,100,RANDOM()) <= 40
                THEN UNIFORM(2,12,RANDOM())
            ELSE 0
        END::DECIMAL(5,2) AS expected_snow_in,
...
CROSS JOIN (
    SELECT DATE_TRUNC('month', DATEADD(month, -SEQ4(), CURRENT_DATE()))::DATE AS month_date
...
CREATE OR REPLACE PROCEDURE TIRECO_DW.ML.SP_GENERATE_FORECASTS()
RETURNS VARCHAR
LANGUAGE SQL
AS
$$
...
BEGIN
    ...
    -- Step 2: Insert forecasts
    INSERT INTO TIRECO_DW.ML.FORECAST_DEMAND (
        model_version,
        sku_id,
        dc_id,
        forecast_date,
        forecast_horizon_days,
        predicted_demand_qty,
        prediction_lower_95,
        prediction_upper_95,
        model_confidence,
    ...
    SELECT 
        'v1.0_baseline' AS model_version,
    ...
    -- ✅ FIXED: use ROW_NUMBER instead of SEQ
    CROSS JOIN (
        SELECT ROW_NUMBER() OVER (ORDER BY 1) AS day_offset
        FROM TABLE(GENERATOR(ROWCOUNT => 28))
    ) horizon

    GROUP BY series_id, horizon.day_offset;

    -- Step 3: Count inserted combinations
    SELECT COUNT(DISTINCT sku_id || dc_id)
    INTO :v_count
    FROM TIRECO_DW.ML.FORECAST_DEMAND
    WHERE forecast_generated_at >= DATEADD(hour, -1, CURRENT_TIMESTAMP());

    RETURN 'Forecasts generated successfully for ' || v_count || ' SKU-DC combinations';
